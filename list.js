function List(board, title, index, dummyList, cards) {
	...
	if (dummyList) {
		...
		for(var i=0; i<this.listCards.length; i++){
			 this.cards[i] = new Card(this,this.listCards[i].title ,this.listCards[i].description,this.listCards[i].priorityLevel,this.listCards[i].attachment,this.listCards[i].status,this.listCards[i].createdAt,this.listCards[i].id);
